...
class ExtractContent(object):
    ...
    def analyse(self, html, opt=None):
        ...
        score = 0
        bodylist = []
        list = re.split(r"""</?(?:div|center|td)[^>]*>|<p\s*[^>]*class\s*=\s*
    ...
    def as_text(self):
        #list = self._strip_tags(self.body)
        #for line in list:
        #    print("line:%s" % line)
    ...
    def extract_title(self, st):
        result = re.search(r"(?s)<title[^>]*>\s*(.*?)\s*</title\s*>", st, re.I)
        #print("prottitle:%s" % result)
        if result != None:
    ...
    def _eliminate_useless_tags(self, html):
        ...
        html = re.sub(r"""\342(?:\200[\230-\235]|\206[\220-\223]|
                \226[\240-\275]|\227[\206-\257]|\230[\205\206])""".encode('utf-8'),
                "".encode('utf-8'), html)
        ...
        html = re.sub(r"<[A-Za-z]{,}[0-9]{,}>", "", html)
        #html = re.sub(r"<a\s+href=(['\"]?)([^\"'\s]+)\1>", "", html, re.I | re.S)
    ...
    def _strip_tags(self, html):
        ...
        #print("%s" % type(st))
        #print("%s\n\n\n" % st)
        st = re.sub(r"\342[\224\225][\200-\277]".encode('utf-8'), "".encode('utf-8'), st)   #keisen
        st = st.decode('utf-8')
        #print("%s" % type(st))
